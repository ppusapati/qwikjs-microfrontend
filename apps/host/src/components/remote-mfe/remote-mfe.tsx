...
import type { RemoteData } from '../../../config';
...
const useFetchOnScroll = (enabled: boolean, url: string) => {
...
const fetchRemoteOnScroll = server$(async (url: string): Promise<string> => {
	const response = await fetchRemote(url);
	return response.ok ? await response.text() : '';
});

const fetchRemote = server$((url: string) => {
...
const getSSRStreamFunction = (remoteUrl: string) => {
